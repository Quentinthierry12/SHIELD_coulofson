...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const m = rows[0];
  if (!m) return NextResponse.json({ error: "Unknown mission." }, { status: 404 });
  ...
  if (status !== undefined) {
    ...
    if (!officer && status !== "completed") {
      return NextResponse.json({ error: "Only an officer can abort or reopen a mission." }, { status: 403 });
    }
    await pool.query(
    ...
    if (status !== "active" && status !== m.status) {
      ...
      for (const t of team) {
        dmByUserId(t.user_id, `🦅 **S.H.I.E.L.D. TRANSMISSION** — Mission **${m.code}** is ${verb}. Stand down.`);
      }
    }
  }
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!rows[0]) return NextResponse.json({ error: "Unknown mission." }, { status: 404 });
