...
export async function POST(req: Request) {
  ...
  if (level >= admin.clearance) {
    return NextResponse.json({ error: `You can only create accounts with clearance below your own (max level ${admin.clearance - 1}).` }, { status: 403 });
  }
  const pool = await db();
...
export async function PATCH(req: Request) {
  ...
  if (id !== admin.id && before[0].clearance >= admin.clearance) {
    return NextResponse.json({ error: "You cannot manage an agent with clearance equal to or above your own." }, { status: 403 });
  }
  ...
  if (!newCodename) return NextResponse.json({ error: "The code name cannot be empty." }, { status: 400 });
